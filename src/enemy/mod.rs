...
use bevy::{prelude::*, time::{self, FixedTimestep}, ecs::schedule::ShouldRun};
use rand::{Rng, thread_rng};
use crate::{GameTextures, WinSize, SPRITE_SCALE, components::{Enemy, SpriteSize, FromEnemy, Laser, Movable, Velocity}, ENEMY_SIZE, EnemyCount, ENEMY_MAX, ENEMY_LASER_SIZE, BASE_SPEED, TIME_STEP};
...
fn enemy_movement_system(
    ...
    for (mut transform, mut formation) in query.iter_mut() {
        ...
        let direction: f32 = if formation.start.0 < 0. { 1. } else { -1. };
        let (x_pivot, y_pivot) = formation.pivot;
        let (x_radius, y_radius) = formation.radius;

        let angle = formation.angle + 
            direction * formation.speed  * TIME_STEP / (x_radius.min(y_radius) * PI / 2.);
